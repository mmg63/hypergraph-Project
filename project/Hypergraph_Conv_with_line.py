...
def load_citation_data():
    ...
    return sparse_H, H, ally, densfeatures
...
class Hyper_Attention_Class(torch.nn.Module):
    def __init__(self,data, edges_weights=None):
        r"""
...
if __name__ == '__main__':
    ...
    data = datasetCora[0]
    # ========== LINE PAPER Idea ==========
    # sparse_H, H, ally, features = load_citation_data()
    # counter = 0
    # edge_weight = torch.zeros(10556)
    # edges = np.array(data.edge_index)
    # for i in range(data.num_nodes):
    #     feature_a = features[i]
    #     neighbors_list = np.where(edges[0] == i)
    #     for j in enumerate(neighbors_list[0].tolist()):
    #         vertex_b = data.edge_index[1, j[1]].item()
    #         feature_b = features[vertex_b, :]
    #         for k in range(data.num_features):
    #             if feature_a[0][0, k] == feature_b[0][0, k] == 1:
    #                 edge_weight[counter] += 1
    #         counter += 1
    #         print('vertex {} to vertext {} is weighted with {}'. format(i, vertex_b, edge_weight[counter-1]))
    # a_file = open('edge_weights', 'wb')
    # torch.save(edge_weight,a_file)
    # #  open edge weight matrix
    a_file = open('edge_weights', 'rb')
    edge_weights_original = torch.load(a_file)
    edge_weights_original += 1 # add one number to the elements value where there is not being zero weights for any edges
    # ===========================
    def train_model():
        model.train()
        optimizer.zero_grad()
    ...
    for trial in range(1, number_of_trials):
        ...
        model = None
        model = Hyper_Attention_Class(data=data, edges_weights=edge_weights_original)
        ...
        plt.grid()
        plt.savefig('./plots/Idea_with_LINE_paper/Line_version_2/Trial_{}_plot_{}_best_val_{}_best_test_{}.png'
