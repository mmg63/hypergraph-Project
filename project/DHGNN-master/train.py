...
def train_test_model():
    ...
    # I add H, as hypergraph incident matrix, by myself
    fts, lbls, idx_train, idx_val, idx_test, n_category, node_dict, edge_dict, H = source(cfg)# loading cora data set from cora-> load_citation_data mustafa
    fts = torch.Tensor(fts).to(device)
    lbls = torch.Tensor(lbls).squeeze().long().to(device)

    # select the model of training mustafa
