...
def load_citation_data():
    ...
    for i in range(len(names)):
        with open("{}/ind.{}.{}".format('Cora/Cora/raw/', 'cora', names[i]), 'rb') as f:
            objects.append(pkl.load(f, encoding='latin1'))

    x, y, tx, ty, allx, ally, graph = tuple(objects)
    test_idx_reorder = parse_index_file("{}/ind.{}.test.index".format("Cora/Cora/raw", "cora"))
...
if __name__ == '__main__':
    ...
    hyperedge_weight_2 = torch.ones([2708], dtype=torch.float)
    hyperedge_weight_2 = hyperedge_weight / 2
    hyperedge_weight = torch.cat((hyperedge_weight, hyperedge_weight_2), dim=-1)
    ...
    for trial in range(1, number_of_trials):
        ...
        plt.grid()
        #plt.savefig('./plots/test_hyperedge_distance2_with hyperedge_weight_immidiate_best_weight/plot_{}.png'.format(trial), dpi=300)
        plt.show()
        # ------------------------------------------------------------------------
