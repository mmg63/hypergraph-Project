"""
تست
طول لایه پنهان 8 نورون
پیاده سازی بر اساس وزن هایپر یال
افزایش در هر ۲۰ مرحله  به اندازه z-score
 کاهش در هر ۵۰ مرحله learning rate بر اساس هر چند اجرا
"""
import torch
...
class Hyper_Attention_Class(torch.nn.Module):
    def __init__(self, data, hyperedge_weight=None, hidden_representation_lenghtion=0, number_of_head=8):
        ...
        #     dropout=0.6
        # )
        # self.hconv3 = hnn.HypergraphConv(
        #     256,
        #     128,
        #     use_attention=True,
        #     heads=1,
        ...
        #     dropout=0.6
        # )
        # self.hconv4 = hnn.HypergraphConv(
        #     128,
        #     64,
        #     use_attention=True,
        #     heads=1,
        ...
        #     dropout=0.6
        # )
        # self.hconv5 = hnn.HypergraphConv(
        #     64,
        #     32,
        #     use_attention=True,
        #     heads=1,
        ...
        #     dropout=0.6
        # )
        # self.hconv6 = hnn.HypergraphConv(
        #     32,
        #     16,
        #     use_attention=True,
        #     heads=1,
        ...
        #     dropout=0.6
        # )
        # self.hconv7 = hnn.HypergraphConv(
        #     16,
        #     8,
        #     use_attention=True,
        #     heads=1,
        ...
        # )
        #
        self.hconv8 = hnn.HypergraphConv(
    ...
    def forward(self, data, hyperedge_weight=None):
        ...
        if hyperedge_weight is None:
            ...
            # x = F.dropout(x, p=0.6, training=self.training)
            # x = F.elu(self.hconv2(x, data.edge_index))
            # x = F.dropout(x, p=0.6, training=self.training)
            # x = F.elu(self.hconv3(x, data.edge_index))
            # x = F.dropout(x, p=0.6, training=self.training)
            # x = F.elu(self.hconv4(x, data.edge_index))
            # x = F.dropout(x, p=0.6, training=self.training)
            # x = F.elu(self.hconv5(x, data.edge_index))
            # x = F.dropout(x, p=0.6, training=self.training)
            # x = F.elu(self.hconv6(x, data.edge_index))
            # x = F.dropout(x, p=0.6, training=self.training)
            # x = F.elu(self.hconv7(x, data.edge_index))
            x = F.dropout(x, p=0.6, training=self.training)
            x = self.hconv8(x, data.edge_index)
        else:
            x = F.dropout(data.x, p=0.6, training=self.training)
            x = F.elu(self.hconv1(x, data.edge_index, hyperedge_weight.t()))
            x = F.dropout(x, p=0.6, training=self.training)
            x = self.hconv8(x, data.edge_index, hyperedge_weight.t())
...
if __name__ == '__main__':
    ...
    # for store validation accuracy during the epochs
    list_evaluate_array =[]
    ...
    # hyperedge_weight_original = hyperedge_degree - hyperedge_degree.min()
    # hyperedge_weight_original = hyperedge_weight_original / (hyperedge_degree.max() - hyperedge_degree.min())
    
    # نرمال سازی وزن‌ هایپریال‌ها بر اساس نرمال سازی استاندارد
    hyperedge_degree = degree(data.edge_index[0], data.x.shape[0], data.x.dtype)
    hyperedge_weight = hyperedge_degree #- torch.mean(hyperedge_degree)
    # hyperedge_weight = hyperedge_weight / (torch.sqrt(torch.std(hyperedge_degree)))
    ...
    def train_model():
        ...
        out = model(data,
                    hyperedge_weight=hyperedge_weight)  # for hypergraph Hyperedge_weight=hyperedge_weight
    ...
    for trains_in_in_specefic_weight in range(0, trains_in_range_weight_iteration):
        ...
        for trial in range(0, number_of_trials):
            ...
            model = Hyper_Attention_Class(data=data,
                                          ...
                                          number_of_head=8)        
            optimizer = torch.optim.Adam(model.parameters(), lr=alpha, weight_decay=5e-4)

            for epoch in range(1, number_of_epochs):
                print("learning rate is : ", alpha)
                #             
                for param_group in optimizer.param_groups: param_group['lr'] = alpha
                ...
                # store validation accuracy to the list
                list_evaluate_array.append(val_)

                if (test_ >= test_best):
                    epoch_best = epoch
                # if(test_>= 0.8):
                #     alpha = 0.0005
                # if(val_ < 0.75 and epoch > 20):
                #     alpha = 0.05
                # else:
                #     alpha = 0.005
                # avg_val_acc = np.average(list_evaluate_array[-5:])
                # # print("average validation accuracy is : {:4f}".format(avg_val_acc))
                # if (val_ > avg_val_acc):
                #     if(tempvar2 ==0):
                #         alpha = alpha * 0.8
                #         print(" learning rate changed to: {:.6f}".format(alpha))
                #         tempvar2 =10
                #     tempvar2 -= 1
                # if (val_ < avg_val_acc):
                #     if (tempvar == 0):
                ...
                # if ((val_ <= 0.79) and ( epoch > 100)):
                #     if tempbool and tempbool2:
                #         alpha = 20
                #         tempbool = False
                #     elif (tempbool2 and not tempbool):
                #         alpha = 0.1
                #         tempbool2 = False
                ...
                log = 'Trial:{} --> Epoch: {:03d} , lr:{:.6f}--> accuracy: Train: {:.4f}, Val: {:.4f}, Test: {:.4f}'
                print(log.format(trial, epoch, alpha, train_, val_, test_))
            ...
            plt.grid()
            plt.savefig('./plots/plots_in_range/_flexible_weight_with_16_neuron_in_hidden_layer/Trial_{}_best_test_{}_hyperedge_weight {:.4f}.png'
                        .format(trial, test_best, hyperedge_weight[0]), dpi=300)
            # plt.show()
            # ------------------------------------------------------------------------


        # print all validation accuracy during trainig and trials
        # print(list_evaluate_array)
