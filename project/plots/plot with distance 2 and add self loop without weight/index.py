...
class Hyper_Attention_Class(torch.nn.Module):
    ...
    def forward(self, data, hyperedge_weight=None):
        ...
        if hyperedge_weight is None:
            ...
            x = F.dropout(x, p=0.6, training=self.training)
            x = self.hconv8(x, data.edge_index, hyperedge_weight)
...
if __name__ == '__main__':
    ...
    data.edge_index = torch.cat((data.edge_index, tempedge), dim=-1)
    ...
    # hyperedge_weight[2708:5416] = 0.3
    ...
    # data.edge_index = torch.cat((data.edge_index, h_2), dim=-1)
    # data.edge_index = torch.cat((data.edge_index, h_2_reverse), dim=-1)
    ...
    for trains_in_in_specefic_weight in range(0, trains_in_range_weight_iteration):
        ...
        for trial in range(0, number_of_trials):
            ...
            model = Hyper_Attention_Class(data=data, hyperedge_weight=None, hidden_representation_lenghtion=8, number_of_head=8)                    
            
            optimizer = torch.optim.Adam(model.parameters(), lr=lr_rate, weight_decay=lmdb_reg)
            
            for epoch in range(1, number_of_epochs):
                if test_best>=0.8:
                    lr_rate = 5e-5
                # print("learning rate is : ", alpha)
                for param_group in optimizer.param_groups: param_group['lr'] = lr_rate
                
                # train_model() ------------------------------------
                model.train()

                out = model(data,hyperedge_weight=None)  # for hypergraph Hyperedge_weight=hyperedge_weight

                loss = F.nll_loss(out[data.train_mask], data.y[data.train_mask])
                # loss = F.cross_entropy(out[data.train_mask], data.y[data.train_mask])
                # loss = torch.nn.MSELoss(out[data.train_mask], data.y[data.train_mask])
                ...
                #     if 'weight' in name:
                #         L2 = L2 + (p**2).sum()
                # loss += lmdb_reg * L2 * 2./data.y.size(0)
                ...
                log = 'Trial:{} --> Epoch: {:04d} , lr:{:.5f}--> accuracy: Train: {:.4f}, Val: {:.4f}, Test: {:.4f}'
                print(log.format(trial, epoch, lr_rate, train_, val_, test_))
            ...
            plt.grid()
            # plt.savefig('./plots/plot with distance 2 and add self loop without weight/Trial_{}_best_test_{}.png'
