"""
آموزش شبکه تنها با هایپریال‌های فاصله دو
هایپریال‌های فصله یک رو ازش کم کردیم
وزن هایپریال‌های فصله دو را اضافه کردیم
به هر هایپریال راس خود را به صورت لوپ اضافه کردیم
در ۸۰۰ ایپاک آموزش رو برای دو بار اجرا می‌کنیم
"""
import torch
...
class Hyper_Attention_Class(torch.nn.Module):
    def __init__(self, data, hyperedge_weight=None, 
                hidden_representation_lenght=0, number_of_head=8):
        r"""
                ...
                """
        self.hidden_representation_lenght = hidden_representation_lenght
    ...
    def forward(self, data, hyperedge_weight=None):
        ...
        if hyperedge_weight is None:
            x = F.dropout(data.x, p=0.6, training=self.training)
            x = self.hconv1(x, data.edge_index)
            x = F.elu(x)
    ...
    for trains_in_in_specefic_weight in range(0, trains_in_range_weight_iteration):
        ...
        for trial in range(0, number_of_trials):
            ...
            model = Hyper_Attention_Class(data=data,
                                          ...
                                          hidden_representation_lenght=8,
                                          number_of_head=8)
            ...
            # plt.grid()
            plt.savefig('./Plot_for_show/Trial_{}_best_test_{}.png'
