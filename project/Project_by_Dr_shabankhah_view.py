...
if __name__ == '__main__':
    ...
    #  و محاسبه وزن‌ها به صورت range normalization and Z-score
    ...
    for i in range(H_2.shape[0]):
        for j in range(H_2.shape[1]):
            if H_2[i, j]:
                ...
                h_2[1, b] = j
                b += 1
    # بدست آوردن زون هایپریال‌های با فاصله دو
    ...
    """
        به منظور اینکه روش استاد شعبان‌خواه را پیاده سازی کنیم   
        پس از اینکه هایپریال‌های با درجه دو را بدست آوردیم
        به هایپریال‌های با درجه یک وزن یکسان ۰.۸ و برای هایپریال‌های با فاصله دو وزن ۰.۲ تخصیص می‌دهیم و شکبه را آموزش می‌دهیم
        ببینیم چی جواب میده بهمون
        نکته: درجه هایپر‌یال‌ها را باید در یک آرایه یک بعدی از جنس تنسور ذخیره کرد 
        که در اینجا یک تنسور ۵۴۱۶ المانی است که ۲۷۰۸ المان آن برای وزن هایپریال‌های با فاصله یک و
        و ۲۷۰۸ المان مابقی برای هایپریال‌های با فاصله دو می‌باشد.
    """
    flexible_hyperedge_weight_1 = 1
    hyperedge_weight = torch.ones([5416], dtype=torch.float)
    # hyperedge_weight = torch.ones([5416], dtype=torch.float)
    # hyperedge_weight[0:2078] = 0.8     #وزن هایپریال‌های با فاصله یک 
    # hyperedge_weight[2708:5416] = 0.2  #وزن هایپریال‌های با فاصله دو
